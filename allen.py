...
def getCoords():
    ...
    for i in range(fromPg, toPg + 1):
        ...
        img = cv2.resize(img, (0, 0), fx=config.scaleFactor,
                         fy=config.scaleFactor)
        ...
        for j in range(boxes):
            ...
            if (isValidQuesNum(quesNum, lastQuesNum)):
                ...
                if (lastQuesCoords > 0):
                    ...
                    # end correction
                    lastQuesCoords = lastQuesCoords - \
                        config.quesFrameShift if lastQuesCoords > config.quesFrameShift else 0
                    y = y - config.quesFrameShift if y > config.quesFrameShift else y

                    cropAndSave(img, i, lastQuesNum,
                                lastQuesCoords, y)
...
def cropAndSave(img, pgNum, quesNum, y1, y2):
    ...
    imgCropped = cv2.resize(imgCropped, (0, 0), fx=config.downScaleFactor,
                            fy=config.downScaleFactor)
    cv2.imwrite(
        f'./output/{chapterName}_Pg{pgNum}_Q{quesNum}.jpg', imgCropped)
